...
import twitter.avro.Coordinate;
import twitter.avro.Entity;
...
public abstract class TweetProcessor {
    ...
    public static Tweet statusToTweet(Status status) {
        ...
        twitter.avro.User.Builder userBuilder = twitter.avro.User.newBuilder();
        ...
        entityBuilder.setHashtags(hashtags);
        entityBuilder.setUrls(urls);
        entityBuilder.setUserMentions(userMentions);
        tweetBuilder.setEntities(entityBuilder.build());

        if (status.getGeoLocation() != null) {
            coordinateBuilder.setLat(status.getGeoLocation().getLatitude());
            coordinateBuilder.setLong$(status.getGeoLocation().getLongitude());
        }
        tweetBuilder.setCoordinate(coordinateBuilder.build());

        twitter4j.User user = status.getUser();
        userBuilder.setDescription(user.getDescription());
        userBuilder.setFollowersCount(user.getFollowersCount());
        userBuilder.setGeoEnabled("".equals(user.getLocation()));
        userBuilder.setId(user.getId());
        userBuilder.setLang(user.getLang());
        userBuilder.setLocation(user.getLocation());
        userBuilder.setScreenName(user.getScreenName());
        userBuilder.setStatusesCount(user.getStatusesCount());
        tweetBuilder.setUser(userBuilder.build());
